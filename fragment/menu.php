
  <nav class="white" role="navigation">
    <div class="nav-wrapper container">
        <img src="materialize/img/UEFA_Euro_2016_logo.png" alt="Logo Euro 2016 - FRANCE">

      <ul class="right hide-on-med-and-down">
          <?php
        echo '<li class="userInfos">'.$_SESSION['monUserCo']['nom'].' '. $_SESSION['monUserCo']['prenom'].' - 0 point<li>';
                ?>
          <ul id='dropdown1' class='dropdown-content'>
    <li><a href="GestionUser.php">Gestion Utilisateur</a></li>
    <li><a href="GestionMatch.php">Gestion Match</a></li>
    <li><a href="Statistique.php">Statistique</a></li>
  </ul>
        <li><a href="home.php">Accueil</a></li>
        <li><a href="mesParis.php">Mes paris</a></li>
         <li><a class="dropdown-button" href="#!" data-activates="dropdown1">Panel Admin<i class="material-icons right">arrow_drop_down</i></a></li>
        <li><a href="logout.php">Déconnexion</a></li>
      </ul>

   
    </div>
  </nav>
